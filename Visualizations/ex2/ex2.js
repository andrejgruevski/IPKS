...
function getPopulation(incomeData){
    ...
    incomeData.forEach(element => {
        population +=element.people;
    })
    return population;
}
function visualizeData(){
    demographicsData.forEach(element => {
        element.population = getPopulation(element.incomeData);
    })
    let pieData = {
        type: "pie",
        labels: demographicsData.map((element) => element.ageRange),
        values: demographicsData.map((element) => element.population),
    }
    let layout = {
        title: "Population distribution by age group"
    }
    Plotly.newPlot("pieChart",[pieData], layout);
    const bubbleTraces = demographicsData.flatMap(group =>
