...
function visualizeData(level) {
    let filterData = employeeData;
    if (level){
        filterData = employeeData.filter(employe => employe.level === level);
    }
    ...
    for (let i = 0; i < filterData.length; i++) {
        traces.push({
            x: filterData[i].earnings.map(emp => emp.month),
            y: filterData[i].earnings.map(emp => emp.amount),
            type: "scatter",
            mode: "lines+markers",
